...
- Issue #137 / PR #138: provider-neutral trusted-principal boundary hardening — validates canonical <=128-character subject/session identifiers, detaches and freezes valid adapter output, and collapses malformed output or resolver failures before tenant persistence/RBAC. Initial head `bcd2c12cb63eeaac2d5ae95916239daa43b2cb15` exposed four stale reference-identity assertions in API Validation run `35662663389`; tests are repaired to assert structurally equal, detached/frozen principals and exact-head reverification is required. Production identity provider selection remains pending and is not activated by this slice.
