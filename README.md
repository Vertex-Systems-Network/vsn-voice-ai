...
- Issue #151: ordinary team-member status lifecycle is the active WU-014 slice — candidate API requires active same-tenant `team.manage`, targets only opaque membership IDs, prohibits self-mutation, allows only `active ↔ suspended`, restricts eligible targets to the ordinary `member` role with baseline non-management permissions, never changes roles/permissions, and reuses the shared fail-closed PostgreSQL runtime; exact-head API + AI-Native verification is required before merge.
