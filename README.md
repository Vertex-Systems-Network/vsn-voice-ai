...
- Issue #149 / PR #150: workspace team privacy hardening is the active WU-014 slice — candidate source keeps raw subject IDs inside the exact tenant-scoped PostgreSQL join only, removes `subject_id` from the browser contract/client, projects bounded nullable workspace `display_name`, and fails closed on malformed/cross-tenant persistence data. Initial head `92cd92cc2277d75e5830370d118b3f13c41d3c82` failed API Validation run `35673636241` at TypeScript compile because the internal duplicate-subject fence still referenced the removed browser field; repaired head `298c7dfe801a27fef191f0b29f3430a3a6d6af31` exposed one stale standalone runtime-team fixture; next repaired head `a94cd2d7a8f1df3d3acf87acdda43856a47cc254` passed API Validation run `35673775985` and Web Validation run `35673775993` but AI Native run `35673776011` exposed a stale Python JSON-schema fixture that still treated `subject_id` as valid. The Python contract now uses `display_name` and explicitly rejects `subject_id`; exact-head API + Web + AI-Native reverification is required before merge.
