...
- Issue #147: browser workspace self profile is the active WU-014 continuation — candidate source performs zero profile requests before explicit workspace selection, uses credentialed no-store GET/PUT with exact tenant-bound response validation, sends only `display_name`/`job_title`, exposes accessible profile states and fences late old-tenant save responses; exact-head Web + AI-Native verification is required before merge.
