...
- Issue #149 / PR #150: workspace team privacy hardening — raw trusted subject IDs remain internal to the exact tenant-scoped PostgreSQL join, the browser team schema/client rejects `subject_id`, bounded nullable workspace `display_name` is projected instead, and malformed/cross-tenant persistence fails closed. Earlier candidate heads exposed stale TypeScript/runtime/Python fixtures; final head `51791feb2047c69b108b143e51742d863dcd9960` passed API Validation run `35673947070`, Web Validation run `35673947111` and AI Native Quality Gates run `35673947096`, then squash-merged as `4b0fedcb72386404b72403208afdca7284cac611`.
- Issue #151 / PR #152: ordinary team-member status lifecycle — active same-tenant `team.manage`, opaque membership targeting, self-mutation denial, `active ↔ suspended` only, ordinary-role/baseline-permission target fencing, no role/permission mutation and shared-pool PostgreSQL persistence. Initial head `43bab4c1f6079dcac63b7cfe0274f70247658203` exposed one TypeScript-only test fixture inference defect; repaired final head `dfc524bfc8b4501f168a723d2c92430417b491a8` passed API Validation run `35768085893` and AI Native Quality Gates run `35768085746`, then squash-merged as `7f4a65c701ce10b56e68113ac0c157ed10f3526c`.
- Issue #153 / PR #154: browser ordinary-member status controls — Team read stays independent, management controls are gated by selected-workspace bootstrap `team.manage`, mutation sends only `{status}`, exact tenant+membership+status response binding is enforced, non-managers remain read-only, and late old-tenant responses are fenced. Exact head `5b01b1ea692cb5a08cd8ef555d2d84a2540cb355` passed Web Validation run `35768752575` and AI Native Quality Gates run `35768752606`, then squash-merged as `e26802cfd2b58125bbf788479de71a21798adda9`.
- Issue #155 / PR #156: browser workspace bootstrap identity privacy hardening — trusted `subject_id` remains inside server authorization/membership resolution only, browser authorization summaries expose organization/membership/roles/permissions, strict schemas/clients reject subject/session fields, and tenant authorization semantics remain unchanged. Exact head `808294c1065935a6741a25cc7813772400a905b2` passed API Validation run `35783954677`, Web Validation run `35783954678` and AI Native Quality Gates run `35783954746`, then squash-merged as `305cce04ad1afc292b3245815791aa981e6e9250`.
- Issue #157 / PR #158: tenant organization display-profile/settings API — persistence contains only `organization_id + display_name`, member reads require `conversation.read`, writes require `team.manage`, request/response schemas are closed and browser-safe, missing rows return an explicit empty name, and the existing shared fail-closed PostgreSQL runtime is reused. Exact head `53b80fb64835aa184f72672b2d900bb504c22362` passed API Validation run `35784705465` and AI Native Quality Gates run `35784705448`, then squash-merged as `b8326fdf2327b721b71c35e0635ed5e865abbca1`.
- Issue #159 / PR #160: browser organization display-profile/settings — zero profile fetches before explicit selection, member-readable display name, `team.manage`-gated edit controls, strict credentialed no-store GET/PUT with exact tenant binding, browser-safe failure states and late old-tenant save fencing. Exact head `1952d9770a1385d9023733d3a20fefbee187dae9` passed Web Validation run `35785225244` and AI Native Quality Gates run `35785225211`, then squash-merged as `db9c13ee7d56055663daa8f55b2048bbdba75161`.
- Issue #161: workspace directory display-name projection is the active WU-014 slice — the subject-bound directory query LEFT JOINs tenant display profiles by exact `organization_id`, browser entries carry bounded nullable `display_name`, selection remains keyed only by stable organization IDs, and subject/session/permissions stay excluded; exact-head API + Web + AI-Native verification is required before merge.
