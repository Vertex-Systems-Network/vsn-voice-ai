...
- Active WU-014 delivery: PRs #134/#135/#136/#138/#140/#142 are merged on `main`; notification-preferences API PR #142 exact-head API Validation run `35671218086` passed 134/134 tests and AI Native run `35671218106` passed before merge `4cdcc9d63f52feef69eea05edfacab86f9c7e711`. Issue #143 is the active browser slice replacing the static Settings placeholder with strict selected-workspace notification GET/PUT UX.
...
- Issue #143 / PR #144: browser notification settings is the active WU-014 continuation — the candidate source removes the static settings placeholder, performs no preference request before explicit workspace selection, strictly validates tenant-bound GET/PUT responses and exposes accessible four-boolean controls without client persistence. Initial head `0d7003a30d2f0322cc3a08ce3a6a01d0c371f439` passed typecheck but Web Validation run `35671506991` failed one stale source-contract assertion that still required the removed placeholder; the assertion is repaired to require the new settings panel and reject the old placeholder, so exact-head reverification is required. Manual exact-head review also found and repaired a tenant-switch race: an in-flight save response is now ignored when the selected organization changes, with browser E2E coverage for the late-response case. Race-regression head `1a5e588cca8d8cfbde9442857c8fee3a0e7a30ec` then failed Web Validation run `35671729498` at TypeScript compile because the test resolver callbacks were nullable under TS 7 control-flow analysis; the test harness now uses definitely-callable initialized resolvers without changing product behavior, and exact-head verification is required again.
