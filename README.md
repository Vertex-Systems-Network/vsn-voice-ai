...
- Active WU-014 delivery: PRs #134/#135/#136/#138/#140/#142/#144/#146 are merged on `main`; self-profile API PR #146 exact-head API Validation run `35672613670` and AI Native Quality Gates run `35672613613` passed before squash merge `258bfe1116e743bc01936f4a63173b64d7789824`. Issue #147 is the active browser slice connecting strict selected-workspace self-profile GET/PUT UX.
...
- Issue #147 / PR #148: browser workspace self profile is the active WU-014 continuation — candidate source performs zero profile requests before explicit workspace selection, uses credentialed no-store GET/PUT with exact tenant-bound response validation, sends only `display_name`/`job_title`, exposes accessible profile states and fences late old-tenant save responses. Initial head `8df51deab71d3853a8e7ed8db8f95fbdc994b04e` passed AI Native Quality Gates run `35672982779`; Web Validation run `35672982865` passed dependency audit/typecheck/contracts/build and both new profile lifecycle cases, but failed the pre-existing signed-out shell count on desktop/mobile because adding the profile panel changed `.area-card` from 6 to 7. The stale assertion is repaired and exact-head Web + AI-Native reverification is required before merge.
