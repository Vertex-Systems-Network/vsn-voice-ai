...
- Active WU-014 delivery: PRs #134/#135/#136/#138/#140 are merged on `main`; security PR #140 exact-head API Validation run `35670636202` passed 120/120 tests with `npm audit` reporting 0 vulnerabilities and AI Native run `35670636113` passed before merge `4f15b0dc0681c09e88bb636b9dbcaf1174b15b7e`. Issue #141 is the active product lane: tenant-bound per-user workspace notification preferences.
...
- Issue #141: persisted notification preferences API is the active WU-014 settings/notifications slice — exact authenticated subject + organization scoping, active membership + `conversation.read` authorization, closed full-replacement booleans, PostgreSQL upsert/default-read behavior, browser-safe response schemas and shared-pool fail-closed runtime are implemented in the candidate source tree and require exact-head API + AI-Native verification.
