...
- Issue #149 / PR #150: workspace team privacy hardening is the active WU-014 slice — candidate source keeps raw subject IDs inside the exact tenant-scoped PostgreSQL join only, removes `subject_id` from the browser contract/client, projects bounded nullable workspace `display_name`, and fails closed on malformed/cross-tenant persistence data. Initial head `92cd92cc2277d75e5830370d118b3f13c41d3c82` failed API Validation run `35673636241` at TypeScript compile because the internal duplicate-subject fence still referenced the now-removed browser response field; the fence now uses the validated raw DB row subject internally without restoring it to the response. Exact-head API + Web + AI-Native reverification is required before merge.
