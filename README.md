...
- Active WU-014 delivery: PRs #134/#135/#136/#138 are merged on `main`; repaired PR #138 exact-head API Validation run `35662807880` passed 120/120 tests and AI Native run `35662807889` passed before merge `d8411a90e9fcda5f9fc4a86340304acbef487a2d`. Issue #139 is the active security-maintenance lane eliminating every resolved Fastify version below 5.12.5 by pinning direct Fastify 5.12.5 and `@nestjs/platform-fastify` 12.0.4.
...
- Issue #139: Fastify security graph hardening — direct `fastify` is pinned to `5.12.5` and `@nestjs/platform-fastify` to `12.0.4`, whose runtime dependency is Fastify `5.12.5`; the committed lock graph contains one resolved Fastify copy and no version below `5.12.5`. Exact-head API + AI-Native verification is required before merge; stale Dependabot PRs #126/#127 will be closed only after replacement merge.
