...
- Active WU-014 delivery: PR #134 cache/test hardening, PR #135 tenant-safe desktop-link lifecycle/status API, and PR #136 browser desktop-link issue/expiry/completion flow are open with their current exact-head technical checks green.
<!-- AI-NATIVE-PROGRESS:END -->

## README Reconciliation Rule — Mandatory

After **every owner query/update** related to this project, the acting AI must reconcile this README against repository reality before finishing. Every **material source-development milestone** must update the AI-Native progress snapshot and any affected dashboard/evidence lines in the **same source commit**. A CI/status-only turn must still reconcile README truth, but must not create a README-only source commit merely to restate remote status because that would create exact-head CI churn.
...
9. `scripts/validate_ai_native_repo.py` must fail when the machine-readable README progress snapshot disagrees with `config/ai/execution-plan.json` or `config/ai/project-state.json`.
10. When material delivery evidence changes without a completed work unit, keep deterministic percentages unchanged but update the README evidence/status narrative in the same material source commit.
