...
- PR #136: browser desktop-link lifecycle, persisted linked-desktop inventory, active-membership tenant selection, workspace access overview, session-state handling and team projection — old head `96d94b3bc381099d8d198686be0bcc34c1e43967` had AI Native Quality Gates green but Web Validation run `35660496084` failed 20 Playwright cases because transient `checking` incorrectly cleared the selected workspace. The repaired branch preserves selection during authentication rechecks and clears it only on terminal signed-out/unavailable states or explicit membership invalidation; exact-head verification is required.
