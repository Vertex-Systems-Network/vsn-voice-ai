...
- Active WU-014 delivery: PRs #134/#135/#136/#138/#140/#142/#144 are merged on `main`; PR #144 exact-head Web Validation run `35671814902` passed 36/36 desktop/mobile E2E cases and AI Native run `35671814964` passed before merge `6d5f4b73031221b307b2ea4c26e62efc67d4584d`. Issue #145 is the active product slice adding a tenant-bound authenticated workspace self-profile API over the shared fail-closed PostgreSQL runtime.
...
- Issue #145: tenant-bound authenticated workspace self profile is the active WU-014 account/profile slice — exact subject+organization persistence, closed bounded `display_name`/`job_title` updates, shared-pool runtime wiring and browser-safe response contracts are implemented on the candidate branch and require exact-head API + AI-Native verification before merge.
