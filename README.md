...
- Issue #143: browser notification settings is the active WU-014 continuation — the candidate source removes the static settings placeholder, performs no preference request before explicit workspace selection, strictly validates tenant-bound GET/PUT responses, exposes accessible four-boolean controls without client persistence, and requires exact-head Web Validation + AI-Native verification.
