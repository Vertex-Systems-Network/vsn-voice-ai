...
NTSTATUS VsnEvtDeviceAdd(WDFDRIVER driver, PWDFDEVICE_INIT device_init) {
    ...
    // This initial control skeleton is intentionally restricted to SYSTEM and
    // built-in Administrators. A least-privilege non-admin broker/interface
    // policy must be designed and verified before production deployment.
...
void VsnEvtDeviceCleanup(WDFOBJECT device_object) {
    auto* context = VsnGetDeviceContext(static_cast<WDFDEVICE>(device_object));
